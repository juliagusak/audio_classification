...
def time_to_batch(x, block_size):
    ...
    with tf.name_scope("time_to_batch"):
        ...
        y.set_shape([
            mul_or_none(shape[0], block_size), mul_or_none(shape[1], 1. / block_size), shape[2]
        ])
...
def conv1d(x, num_filters, filter_length, name, dilation = 1, causal = True, 
    ...
    with tf.name_scope(name):
        weights = tf.get_variable(
            '{}/W'.format(name), shape=kernel_shape, initializer = kernel_initializer)
        biases = tf.get_variable(
            '{}/biases'.format(name), shape=biases_shape, initializer = biases_initializer)

        tf.summary.histogram('{}/W'.format(name), weights)
        tf.summary.histogram('{}/biases'.format(name), biases)
        ...
        x_4d = tf.reshape(x_ttb, [tf.shape(x_ttb)[0], 1,
        ...
        y = tf.reshape(y, [tf.shape(y)[0], y_shape[2], num_filters])
...
def pool1d(x, window_length, name, mode='avg', stride=None):
    ...
    _, length, num_channels = x.get_shape().as_list()
    ...
    with tf.name_scope(name):
        ...
        pooled_reshape = tf.reshape(pooled, [batch_size, length // stride, num_channels])
...
def fc_layer(input, size_in, size_out, name = "fc"):
    with tf.name_scope(name):
        w = tf.get_variable("{}/W".format(name), initializer = tf.truncated_normal(shape = [size_in, size_out], stddev = 0.1))
        b = tf.get_variable("{}/B".format(name), initializer=tf.constant(0.1, shape = [size_out]))
        act = tf.matmul(input, w) + b
        
        tf.summary.histogram('{}/W'.format(name), w)
        tf.summary.histogram('{}/biases'.format(name), b)
        tf.summary.histogram('{}/activations'.format(name), act)
