...
import opts_pytorch
from datasets_pytorch import BCDatasets, tensor_to_numpy, get_train_transform, get_test_transform, IMG, LABEL
from draw_process import draw_progress
from models_pytorch import weights_init
...
def create_model(opt):
    model = getattr(models_pytorch, opt.netType)(opt.nClasses)
    if use_cuda(opt):
...
def train(model, optimizer, loss_func, training_generator):
    # train model
    model.train(True)
    epoch_train_loss = []
    ...
    for batch in training_generator:
        ...
        loss = loss_func(output, labels.type(FloatTensor))
        loss.backward()
...
if __name__ == '__main__':
    ...
    for test_fold in opt.splits:
        ...
        for epoch in range(opt.nEpochs):
            ...
            mean_train_error_rate = accuracy_on_batch(epoch_error_rate, training_generator, opt.batchSize)
            mean_val_error_rate = accuracy_on_batch(val_epoch_rate, validation_generator, 1)
            train_error_rate.append(mean_train_error_rate)
            val_error_rate.append(mean_val_error_rate)
            
            print("| Epoch: {:4d}| Fold: {:d}| train_loss: {:2.3f} | test_loss: {:2.3f} | train_acc: {:2.3f} | test_acc: {:2.3f}|".format(epoch, test_fold, mean_train_loss, mean_val_loss, mean_train_error_rate, mean_val_error_rate))
